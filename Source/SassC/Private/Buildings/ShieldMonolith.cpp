...
bool AShieldMonolith::SpawnBeamEmitter_Validate(UParticleSystem* PSysToSpawn, AShieldMonolith* SourceMonolith,
                                                AShieldMonolith* TargetMonolith)
{
	return true;
}
...
void AShieldMonolith::DestroyConnectedBeamEmitters_Implementation()
{
	for (UParticleSystemComponent* PSys : AttachedParticleSystems)
	{
		if (PSys)
		{
			PSys->DeactivateSystem();
			PSys->DestroyComponent();
		}
	}
}
...
float AShieldMonolith::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator,
                                  AActor* DamageCauser)
{
...
TArray<AShieldMonolith*> AShieldMonolith::FindMonolithsInRange(float Range, TArray<AActor*> ActorsToIgnore) const
{
	...
	{
		if (!WallsInRange.Contains(Hit.GetActor()) && Hit.GetActor()->IsA(AShieldMonolith::StaticClass()) && Hit.
			GetActor() != this)
		{
